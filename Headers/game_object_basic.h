...
private:
	...
	std::shared_ptr<Mesh> process_mesh(aiMesh* mesh, const aiScene* scene,const std::string& path)
	{
		...
		std::vector<unsigned int> indices;
		std::vector<Texture> textures;
		material_properties mat_props = { false, glm::vec3(1.0f), glm::vec3(1.0f), glm::vec3(1.0f), 32.0f, glm::vec3(0.0f), 1, 1, 0 };
		//process vertices
...
public:
	...
	//so you need to manage meshes through pointers becouse vectors copy elements when resized
